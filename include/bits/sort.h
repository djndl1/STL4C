...
#define basic_comparator_register(type)                                        \
  bool stl4c_##type##_less(const type a, const type b) { return a < b; }       \
                                                                               \
  bool stl4c_##type##_equal(const type a, const type b) { return a == b; }     \
                                                                               \
  bool stl4c_##type##_greater(const type a, const type b) { return a > b; } \

#define sort_type_register(type, container)     \
ITER_TYPE(type, container) stl4c_##type##_##container##_certain_element(ITER_TYPE(type, container) first,\
